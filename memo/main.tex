...
\begin{itemize}
  ...
  \begin{enumerate}
    ...
    $\varPhi\colon R \to R'$.
    \item
    This completes existence part.
    \item
    (Uniqueness part):
    Let $\varPhi\colon R\to R'$ satisfies the conditions.
    \item Let $a\in R \setminus F$.
    % \item ???
    % Because $R$ is an algebraic extension of $F$,
    % $[R:F] < \infty$ (Yukie 3.1.18).
    % By primitive element theorem, there exists $a\in R\setminus L$ such that
    % $R = F(a)$.
    \item
    Let $f = \sum_{i=0}^q c_i X^i \in F[X]$ be a minimal polynomial of $a$.
    \item
    Let roots of $f$ be $a_1 < \dots < a_n$ and $a_j = a$.
    \item
    By the corollary of Strum theorem (Cor. 1.2.12),
    \begin{align}
      n &= \# \set{x\in R; f(x)=0}\\
      & = v(f,1;-\infty)-v(f,1;+\infty)\\
      &=
      v(f_\varPhi,1;-\infty)-v(f_\vaPhi,1;+\infty)\\
      &=
      \# \set{x\in R'; f_\varPhi(x)=0}.
    \end{align}
    \item
    By (12),
    let roots of $f_\varPhi$ be $b_1 < \dots < b_n$.
    \item
    $\varPhi(a_1),\dots,\varPhi(a_n)$ are roots of $f_\varPhi$.
    \item
    By monotone of $\varPhi$,
    \begin{align}
      \varPhi(a_1) \le \dots \le \varPhi(a_n).
    \end{align}
    \item
    By (13,14,15), $b_i = \varPhi(a_i)$.
    \item
    This determines where $a$ goes to, that is $b_j$.
    Uniquness is proved.
  \end{enumerate}
  \item (Theorem 1.3.2):
  \begin{enumerate}
  ...
  \begin{itemize}
    \item
    \begin{align}
      \#\set{\text{orderings of $F(a)$ extending the ordering of $F$}} =
      \# \set{x\in (\text{the real closure of $(F,\le)$}) ; f(x)=0}.
    \end{align}
    \item
    \begin{align}
      (\text{the above number}) \equiv [F(a):f] \mod 2.
    \end{align}
  \end{itemize}
  \item
  \pf
  \begin{enumerate}
    \item (First half):We will construct the bijection between
    \begin{align}
      \set{\text{orderings of $F(a)$ extending the ordering of $F$}} \simeq
       \set{x\in (\text{the real closure of $(F,\le)$}) ; f(x)=0}.
    \end{align}
    \begin{enumerate}
      \item ($\rightarrow$):
      Fix an ordering in $\set{\text{orderings of $F(a)$ extending the ordering of $F$}}$.
      \item
      Flow $a$ along the function
      \begin{align}
        (F(a),\le) \to (\text{the real closure of $F(a)$})
        \xrightarrow{\text{Prop 1.3.4, iso}}
        (\text{the real closure of $F$}).
      \end{align}
      \item
      The above image of $a$ is a root of $f$ because $f$ is a minimal polynomial.
      The image is $\in \set{x\in (\text{the real closure of $(F,\le)$}) ; f(x)=0}$.
      \item ($\leftarrow$):
      Let $R$ be the real closure of $(F,\le)$.
      \item
      Let $b\in \set{x\in (\text{the real closure of $(F,\le)$}) ; f(x)=0}$.
      \item
      Define an $F$-morphism $\varPhi\colon F(a)\to R$ by $\varPhi(a)  = b$.
      This is well-defined because $a$ and $b$ are roots of $f$.
      \item
      $\varPhi$ is an inclusion, so the ordering of $F(a)$ is induced by the limitation of
      $R$.
    \end{enumerate}
    \item (Last half):
    If $x\in \ol F$ is a root of $f$, but not in $R$,
    $\ol x$ is also an root of $f$, so the number of such a roots
    don't affect $[F(a):f] \bmod 2$.
    If $x\in R$ and $f(x)=0$ then it changes $[F(a):f]$ by 1.
  \end{enumerate}
\end{itemize}
